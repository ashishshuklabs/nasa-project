...
// for all
const useServerData = (id: string): { loaderScript: ReactNode; value: any } => {
  ...
  const loadScript = (id: string, data: any) => (
    <script
      ...
      dangerouslySetInnerHTML={{
        __html: `
        window.ChunkData =${JSON.stringify({[id]: globalCache.get(id)})}
    `,
      }}
    />
  );
  ...
  if (isServer) {
    ...
    const data: Pokemon = getFetcher.read();
  ...
  console.log("this is client request, returning....", window.ChunkData && window.ChunkData[id], 'id si......', id);
  return {
    loaderScript: null,
    value: window.ChunkData ? window?.ChunkData[id]: null,
  };
};
